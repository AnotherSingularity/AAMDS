...
fn single_clean_track_is_deterministic_across_runs() {
    ...
    let out1 = run_pipeline(&s1.manifest.name, &mut s1.adapter, s1.policy.clone(), far_future).unwrap();
    let mut s2 = single_clean_track();
    let out2 = run_pipeline(&s2.manifest.name, &mut s2.adapter, s2.policy.clone(), far_future).unwrap();
    assert_eq!(trace_digest(&out1), trace_digest(&out2), "same inputs must yield same trace");
...
fn different_scenarios_produce_different_digests() {
    ...
    let out_a = run_pipeline(&a.manifest.name, &mut a.adapter, a.policy.clone(), far_future).unwrap();
    let mut b = crossing_two_tracks();
    let out_b = run_pipeline(&b.manifest.name, &mut b.adapter, b.policy.clone(), far_future).unwrap();
